...
static void ble_evt_handler(ble_evt_t const * p_ble_evt, void * p_context)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GAP_EVT_CONNECTED:
...
#endif /* #if (PEER_MANAGER_ENABLED == 1) */
#if defined(ICM40609)
            // Initialize ICM40609 with default 5 ms settings (timer-based sampling).
            // Only one device present - try all addresses
            for (uint8_t addridx = ICM40609_I2C_ADDR0; addridx <= ICM40609_I2C_ADDR1; addridx++) {
              err_code = icm40609_init(&m_icm40609[0u], &m_twi, addridx);
            ...
            NRF_LOG_INFO("ICM40609 initialized (timer mode, 5 ms sample period)");
#endif
#if defined(ADS1299)
            ads1299_start_conv();
            ads1299_start_rdatac(&m_eeg[0]);
#if (NUM_OF_ADS1299 == 2)
            // ads1299_wakeup(&m_eeg[1]); // TODO wakeup the second ADS1299 device only to save power
            // nrf_delay_ms(10); // TODO add delay to ensure the second ADS1299 device is awake before starting RDATAC mode
