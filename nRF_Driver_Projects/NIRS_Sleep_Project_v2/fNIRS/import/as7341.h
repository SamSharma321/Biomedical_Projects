 ...
 */
typedef enum {
  AS7341_GPIO_OUTPUT, ///< The GPIO pin is configured as an open drain output
...
public:
  ...
  bool readAllChannels_ori(uint16_t *readings_buffer);
  bool readCurrentMUXChannels(uint16_t *readings_buffer);
...
// void as7341_begin();
void as7341_init();
int as7341_read(uint8_t addr, uint8_t *buf);
