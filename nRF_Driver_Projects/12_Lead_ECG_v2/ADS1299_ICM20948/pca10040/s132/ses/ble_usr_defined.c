...
static void ble_evt_handler(ble_evt_t const * p_ble_evt, void * p_context)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GAP_EVT_CONNECTED:
            ...
            for (uint8_t i = 0; i < NUM_OF_ICM_DEVICES; i++)
            {
                // Initialize ICM20948 with default 5 ms settings (timer-based sampling).
                err_code |= icm20948_init(&m_icm20948[i], &m_twi, ICM20948_I2C_ADDR0 + i);
                if (err_code)
                   NRF_LOG_INFO("ICM Device @ index = %u failed! Device did not respond.\n", i);
                m_icm[i].icm_sample_count = 0u;
            }
            if (!err_code)
                NRF_LOG_INFO("ICM20948 initialized (timer mode, 5 ms sample period)");
#endif /* #if defined(ICM20948) */
#if defined(ADS1299)
            ads1299_start_conv();
