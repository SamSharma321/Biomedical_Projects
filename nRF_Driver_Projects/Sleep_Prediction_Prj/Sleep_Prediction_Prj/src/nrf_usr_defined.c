...
void gpio_init(void)
{
    ...
    nrf_gpio_pin_dir_set(MAX30102_INT_PIN, NRF_GPIO_PIN_DIR_INPUT);

    nrf_drv_gpiote_in_config_t max_in_config = GPIOTE_CONFIG_IN_SENSE_HITOLO(is_high_accuracy);
    max_in_config.is_watcher = true;
    max_in_config.pull = NRF_GPIO_PIN_NOPULL;

    err_code = nrf_drv_gpiote_in_init(MAX30102_INT_PIN, &max_in_config, max30102_in_pin_handler);
    APP_ERROR_CHECK(err_code);
    nrf_drv_gpiote_in_event_enable(MAX30102_INT_PIN, true);
#endif
