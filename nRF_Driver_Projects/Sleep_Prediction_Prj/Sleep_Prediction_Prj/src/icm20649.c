...
#define ICM20649_TWI_TIMEOUT_MS 20u
...
static void icm20649_twi_handler(nrf_drv_twi_evt_t const *p_event, void *p_context);
static ret_code_t icm20649_twi_recover(void);
static ret_code_t icm20649_twi_wait_for_completion(void);
static ret_code_t icm20649_twi_tx(nrf_drv_twi_t const *twi,
                                  uint8_t address,
                                  uint8_t const *p_data,
                                  uint8_t length,
                                  bool no_stop);
static ret_code_t icm20649_twi_rx(nrf_drv_twi_t const *twi,
                                  uint8_t address,
                                  uint8_t *p_data,
                                  uint8_t length);

static volatile bool m_twi_xfer_done = false;
static volatile ret_code_t m_twi_xfer_result = NRF_SUCCESS;
static nrf_drv_twi_config_t const m_twi_icm_config = {
...
void twi_init(void)
{
    ret_code_t err_code = nrf_drv_twi_init(&m_twi, &m_twi_icm_config, icm20649_twi_handler, NULL);
...
ret_code_t twi_write_bytes(uint8_t address, uint8_t const *data, size_t len, bool no_stop)
{
    if (data == NULL)
    {
    ...
    return icm20649_twi_tx(&m_twi, address, data, (uint8_t)len, no_stop);
}

ret_code_t twi_read_bytes(uint8_t address, uint8_t *data, size_t len)
{
    if (data == NULL)
    {
        return NRF_ERROR_NULL;
    ...
    return icm20649_twi_rx(&m_twi, address, data, (uint8_t)len);
}

ret_code_t twi_write_read(uint8_t address,
                          uint8_t const *tx_data,
                          size_t tx_len,
                          uint8_t *rx_data,
                          size_t rx_len)
{
    ret_code_t err = twi_write_bytes(address, tx_data, tx_len, true);
    if (err != NRF_SUCCESS)
    {
        return err;
    }

    return twi_read_bytes(address, rx_data, rx_len);
}
...
static void icm20649_twi_handler(nrf_drv_twi_evt_t const *p_event, void *p_context)
{
    ...
    switch (p_event->type)
    {
        case NRF_DRV_TWI_EVT_DONE:
            m_twi_xfer_result = NRF_SUCCESS;
            break;

        case NRF_DRV_TWI_EVT_ADDRESS_NACK:
            m_twi_xfer_result = NRF_ERROR_NOT_FOUND;
            break;

        case NRF_DRV_TWI_EVT_DATA_NACK:
            m_twi_xfer_result = NRF_ERROR_INVALID_DATA;
            break;

        default:
            m_twi_xfer_result = NRF_ERROR_INTERNAL;
            break;
    }
...
static ret_code_t icm20649_twi_recover(void)
{
    ...
    ret_code_t err = nrf_drv_twi_init(&m_twi, &m_twi_icm_config, icm20649_twi_handler, NULL);
    if (err == NRF_SUCCESS)
    {
        nrf_drv_twi_enable(&m_twi);
    }
...
static ret_code_t icm20649_twi_wait_for_completion(void)
{
    uint32_t const start_ticks = app_timer_cnt_get();
    uint32_t const timeout_ticks = APP_TIMER_TICKS(ICM20649_TWI_TIMEOUT_MS);

    while (!m_twi_xfer_done)
    {
        if (app_timer_cnt_diff_compute(app_timer_cnt_get(), start_ticks) >= timeout_ticks)
        {
            NRF_LOG_WARNING("ICM20649: TWI transfer timed out after %u ms.", ICM20649_TWI_TIMEOUT_MS);
            (void)icm20649_twi_recover();
            return NRF_ERROR_TIMEOUT;
        }
        __NOP();
    }

    return m_twi_xfer_result;
}
...
{
    m_twi_xfer_done = false;
    m_twi_xfer_result = NRF_ERROR_BUSY;

    ret_code_t err = nrf_drv_twi_tx(twi, address, p_data, length, no_stop);
    if (err != NRF_SUCCESS)
    {
        return err;
    }

    err = icm20649_twi_wait_for_completion();
    if ((err != NRF_SUCCESS) && (err != NRF_ERROR_TIMEOUT))
    {
        (void)icm20649_twi_recover();
    }
                                  ...
                                  uint8_t length)
{
    ...
    ret_code_t err = nrf_drv_twi_rx(twi, address, p_data, length);
    if (err != NRF_SUCCESS)
    {
        return err;
    }
...
ret_code_t icm20649_select_bank(icm20649_t *dev, uint8_t bank)
{
    ...
    ret_code_t err = icm20649_twi_tx(dev->twi, dev->address, buf, sizeof(buf), false);
    if (err == NRF_SUCCESS)
    {
...
ret_code_t icm20649_write_regs(icm20649_t *dev, icm20649_reg_t reg, const uint8_t *data, size_t len)
{
    ...
    return icm20649_twi_tx(dev->twi, dev->address, buf, (uint8_t)(len + 1u), false);
}
...
ret_code_t icm20649_read_regs(icm20649_t *dev, icm20649_reg_t reg, uint8_t *data, size_t len)
{
    ...
    err = icm20649_twi_tx(dev->twi, dev->address, &addr, 1u, true);
    CHK_FN_ERR_DATA(err);

    return icm20649_twi_rx(dev->twi, dev->address, data, (uint8_t)len);
}
