...
#define BLE_PHY_2MBPS_TX_DBM_MAX 4
#endif
...
#else
#define BLE_DYN_LOG_INFO(...) \
    do                        \
    {                         \
    } while (0)
#define BLE_DYN_LOG_WARNING(...) \
    do                           \
    {                            \
    } while (0)
#endif
...
#define ICM_CAL_SAMPLES_IMU1 200u
#define ICM_CAL_DELAY_MS_IMU1 2u
#define ICM_CAL_SAMPLES_IMU2 10u
#define ICM_CAL_DELAY_MS_IMU2 40u
...
static uint8_t m_ble_phy_current = BLE_GAP_PHY_2MBPS;
...
#endif
#if (ICM20948_PRESENT)
    adv_uuids[adv_uuid_count].uuid = BLE_UUID_BIOPOTENTIAL_IMU_MEASUREMENT_SERVICE;
    adv_uuids[adv_uuid_count].type = BLE_UUID_TYPE_BLE;
    adv_uuid_count++;
#endif
...
#endif /* #if (PEER_MANAGER_ENABLED == 1) */
#if defined(ADS1292)
        ads1291_2_main_init(false);
        NRF_LOG_INFO("ADS129x device(s) configured successfully.");
#endif
#if (ICM20948_PRESENT)
        icm20948_twi_init();
        err_code = icm20948_init(&m_icm20948[0u], ICM20948_I2C_ADDR0); // ICM device 1
        APP_ERROR_CHECK(err_code);
        err_code = icm20948_apply_profile(&m_icm20948[0u], ICM20948_PROFILE_SWALLOW_IMU1);
        APP_ERROR_CHECK(err_code);
        err_code = icm20948_calibrate_bias(&m_icm20948[0u], ICM_CAL_SAMPLES_IMU1, ICM_CAL_DELAY_MS_IMU1);
        APP_ERROR_CHECK(err_code);
        err_code = icm20948_fifo_configure(&m_icm20948[0u], true, true, false);
        APP_ERROR_CHECK(err_code);
#if (NUM_ICM_DEVICES == 2u)
        err_code = icm20948_init(&m_icm20948[1u], ICM20948_I2C_ADDR1); // ICM device 2
        APP_ERROR_CHECK(err_code);
        err_code = icm20948_apply_profile(&m_icm20948[1u], ICM20948_PROFILE_CONTEXT_IMU2);
        APP_ERROR_CHECK(err_code);
        err_code = icm20948_calibrate_bias(&m_icm20948[1u], ICM_CAL_SAMPLES_IMU2, ICM_CAL_DELAY_MS_IMU2);
        APP_ERROR_CHECK(err_code);
        err_code = icm20948_fifo_configure(&m_icm20948[1u], true, true, false);
        APP_ERROR_CHECK(err_code);
#endif /* #if (NUM_ICM_DEVICES == 2) */
        NRF_LOG_INFO("ICM device(s) configured, bias-calibrated, and FIFO-enabled successfully.");
        m_icm_rr_next_idx = 0u;
        icm_timers_start();
        NRF_LOG_INFO("ICM Timers configured.");
...
#endif
#if (ICM20948_PRESENT)
        uint16_t tx_complete_count = p_ble_evt->evt.gatts_evt.params.hvn_tx_complete.count;
        for (uint16_t i = 0u; i < tx_complete_count; i++)
        {
            icm_flush_pending_round_robin();
        }
#endif
        break;
...
#if defined(ADS1292)
    ble_eeg_on_ble_evt(&m_eeg, (ble_evt_t *)p_ble_evt);
#endif

#if (ICM20948_PRESENT)
    ble_icm_on_ble_evt(&m_icm[0], (ble_evt_t *)p_ble_evt);
#if (NUM_ICM_DEVICES == 2)
    ble_icm_on_ble_evt(&m_icm[1], (ble_evt_t *)p_ble_evt);
#endif /* #if (NUM_ICM_DEVICES == 2) */
#endif
}
